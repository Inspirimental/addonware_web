...
const QuestionnaireTransformation = () => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (!responseData) throw new Error("No response data returned");

      // Now insert all answers
      const answerInserts = Object.entries(answers).map(([questionId, value]) => {
        ...
        if (question.question_type === 'multiple_choice') {
          ...
          const ratingValue = value.includes('/') ? value.split('/')[0] : value;
          answerData.answer_number = parseInt(ratingValue);
      ...
      await submitQuestionnaireResponse(
        ...
        answers,
        questions,
        questionnaire!.notification_email || undefined
      );
...
export default QuestionnaireTransformation;